...
impl TagStore {
    ...
    pub fn already_indexed_by_metadata(
        &self, path: &str, size: u64, mtime: u64,
    ) -> SqlResult<bool> {
        let conn = self.connect()?;
        let mut stmt = conn.prepare(
        ...
        let count: i64 = stmt.query_row(
            params![path, size as i64, mtime as i64], |r| r.get(0)
        )?;
        Ok(count > 0)
    }
    ...
    pub fn upsert_document(
        &self, content_hash: &str, file_path: &str, file_type: &str,
        file_size: i64, modified_ts: i64,
    ) -> SqlResult<()> {
        let conn = self.connect()?;
        conn.execute(
...
mod tests {
    ...
    fn create_and_list_tags() {
        ...
        conn.execute("INSERT INTO tags (name) VALUES ('tax')", []).unwrap();
        conn.execute("INSERT INTO tags (name) VALUES ('receipt')", []).unwrap();
